...
def home(request):
    ...
    # ✅ CORREGIDO: Solo productos de vendedores
...
def add_to_cart(request, product_id):
    ...
    if not created:
        if cart_item.quantity + 1 > product.stock:
            messages.error(request, f"No hay suficiente stock de {product.name}. Solo hay {product.stock} unidades.")
            return redirect('cart_detail')
    ...
    if request.method == 'POST':
        ...
        if quantity > 0:
            ...
            if quantity > item.product.stock:
                messages.error(request, f"No hay suficiente stock. Solo hay {item.product.stock} unidades de {item.product.name}.")
                return redirect('cart_detail')
            item.quantity = quantity
            item.save()
            messages.success(request, f"Cantidad de {item.product.name} actualizada a {quantity}")
        else:
            item.delete()
